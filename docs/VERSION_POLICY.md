...
Which leaves one line to draw, because this specification does land
document-level refusals and calls them MINOR (SPEC-14's repeated top-level key,
SPEC-36's forbidden line breaks) while SPEC-32 leans on the paragraph above to
forbid one. They are reconcilable, and the reconciling sentence is this: a
DOCUMENT-level refusal is permitted only for bytes that have NO conforming
reading at all, and every refusal grounded in a VALUE a reader does not
recognise is ELEMENT-level. The test is not how serious the defect looks, it is
whether a conforming reading exists. A repeated top-level key has none (PyYAML
keeps the last value, yaml-cpp the first, and nothing downstream can tell); a
forbidden line break has none (a parser breaks a line where the grammar does
not, so the two build different documents); an unrecognised member of a closed
vocabulary has one, so a reader that cannot use it is incomplete, must say so,
and must carry on. That makes SPEC-14, SPEC-32 and SPEC-36 one rule rather than
three precedents, and it is the sentence to quote the next time this argument
comes up.
...
`chiplet_format_io.__version__` / the C++ `READER_RELEASE` is the release of the
READER, never of the format, and it moves for reasons the documents on disk know
nothing about. A consumer that vendors a copy gates on it (see [Byte identity of
vendored copies is a tripwire, not the
contract](#byte-identity-of-vendored-copies-is-a-tripwire-not-the-contract)), so
each move is written down here with what a consumer gets by taking it.

| Release | What moved, and why it is a READER minor rather than a format one |
|---|---|
| 1.4.0 | The top-level block grammar's refusal generalised, in both readers, and both writers gained a matching refusal on their complete output (SPEC-41). The reader half: a line at column zero that is not a key line and not one of the attributable shapes (blank, `#` comment, `%` directive, `---` / `...` marker, block sequence entry) now makes a document NOT SPLITTABLE, where before only the QUOTED spelling did. The writer half is why the reader half was needed at all: measured on PyYAML 6.0.3, `dumps()` given a top-level key it cannot write bare emitted a document its own splitter mis-read, with no diagnostic on either side. A key carrying NEL, LS, PS or a CR came out as an explicit key (`? "a\Lb"` on one line, `: x: 1` on the next) and a key carrying a space came out bare as `a b:`; in both, the split reported two top-level keys, `loads` reported three, and the extra block was attributed to `assembly`, whose owner drops it on the next re-export. That is the SPEC-36 property (split and parse disagree) produced by our own writer, so `dumps()` now splits the text it just emitted and refuses when the split does not return exactly the keys it was given, in order. MINOR and not PATCH because a consumer can OBSERVE both refusals: the splitting accessors (`top_level_blocks`, `top_level_block`, and the C++ `flow_source`) refuse documents they used to split, and `dumps()` refuses mappings it used to write. Two of those moved verdicts are in the shared oracle and are named there rather than left to be discovered: `flow :` and a whole document in flow style are now not splittable, not merely undelimitable. Not a format change: no legal document's BYTES or MEANING moved, reading is unchanged in both readers, and the sequence-entry exemption is what keeps that true (PyYAML writes a block sequence at the parent's indentation, so 68 of the 70 column-zero non-key lines across the 77 tracked `.chiplet` in this ecosystem are `- id: ...` entries under `components:`, and a rule without the exemption would refuse to split almost everything we ship). The writer-output digest did NOT move, and that is asserted rather than claimed: `conformance/fixtures/writer_output_pin.json` names release 1.4.0 with the unchanged sha256 from 1.3.0. At that matching release, `conformance/test_writer_output_pin.py` rejects any changed byte; a one-byte perturbation is the negative control. The C++ check includes caller-supplied flow source rather than assuming all column-zero keys originate in its fixed mapping. |
| 1.3.0 | The Python writer stopped spelling U+0085 as `\N` and now writes `\x85`. Measured on PyYAML 6.0.3 and yaml-cpp 0.8.0: `"a\Nb"` reads back as `61 c2 85 62` in Python and `61 85 62` in C++, a bare 0x85 that is not valid UTF-8 on its own, so a document this project's Python writer produced handed this project's C++ reader a malformed string. `\L` and `\P` round-trip identically in both and are unchanged; the defect was one escape, not the family. A MINOR and not a PATCH because a consumer CAN observe it: the bytes our writer emits for such a value changed, and at least one consumer runs a byte-exact writer parity gate. Reading is unaffected in both directions, since both spellings decode to U+0085 in both readers, so nothing on disk becomes illegal and no document's meaning moves. THE REASON THIS ROW EXISTS AT ALL is a process defect worth naming: the change landed first with the release left at 1.2.0, so two legitimate vendored copies, one from each commit, declared the same release over different bytes. The ecosystem's contract registry treats exactly that as DRIFTED, precisely so an in-place edit cannot be invisible, and a CONSUMER caught it during a re-vendor rather than this repository catching it on the way out. The rule the miss implies: when the declared artifact's bytes move, the declared version moves with them, and the deliberate no-bump of the SPEC-21 row above is the narrow exception, allowed only because nothing a consumer can observe moved with it. |
| 1.2.0 | The readers stopped refusing an unrecognised `interfaces[].type` (SPEC-32, panel ruling Q1): they carry the string, report it through `on_warn` at parse, and refuse nothing over it. Rule 4 also moved TIER in the C++ reference, from the parser to the validator, which is what the spec's own rule text always said; it had been enforced inside `parse_interface`, a function taking no options, so `LoadOptions::validate = false` never reached it and the two readers loaded different documents from one file (SPEC-34). `kKnownInterfaceTypes` and `is_known_interface_type` are now exported from the C++ header, because a consumer cannot refuse the ELEMENT without the list, and element-refusal is what makes the MINOR label true rather than aspirational. `on_warn` is declared the single NORMATIVE channel in both readers, undeduplicated, with `ChipletDocument::warnings` and the Python stdlib `warnings` emission named as non-normative convenience. Not a format change by this policy's own test: no document legal today becomes illegal, the accepted set strictly grows, nothing on disk changes, and a consumer that keeps its own refusal stays correct. The format MINOR already owed for `solder_bump` at 1.1 is untouched and stays owed. |
...
| Line breaks fixed to LF and CRLF; NEL, `U+2028`, `U+2029` and a lone CR ill-formed | this slice (SPEC-36) | An INTENTIONAL behaviour change, stated as one rather than filed as a fix: `name: demo<U+2028>trailing` loads in yaml-cpp 0.8.0 today and stops loading after this release, and consumers meet it through a vendoring bump. The set is every character a YAML parser breaks a line on that the top-level block grammar does not: NEL, `U+2028` and `U+2029` anywhere, and `U+000D` unless an LF follows it immediately, end of file included. It is not a MAJOR because the format had never defined its line-break set, so those bytes were never legal in it; yaml-cpp accepting them was implementation behaviour, and PyYAML refused the same bytes. It is a DOCUMENT-level refusal and still a MINOR under the line drawn above: these bytes have no conforming reading at all, which is the only thing that permits one. The disagreement runs both ways (the smuggle shape loads in PyYAML as a second top-level `format_version` and throws in yaml-cpp), so there was never a reading two conforming readers shared, which is the same test the repeated-key refusal passed. The safety argument is NOT "the population is zero": a sweep bounds the corpus it walked, and no corpus we can walk covers build trees, what a user pastes into a GUI (LS and PS are what a paste from a web page carries), the documents held by the KiCad fork's users, or git history. It is that no producer of ours can emit one of these characters raw once the writer rule is in, and that the escaped form carries any value that needs one, so a document that meets this refusal was not written by a conforming producer. Measured: all 19 `.chiplet` in chiplet-spec, fixtures and examples, dumped through the writer before and after the rule give zero differing files. The sweeps corroborate and are named for what they covered: every `.chiplet` outside build trees plus the YAML and stackups shipped with them (none of the three); the KiCad plugin's 187 `.chiplet` (none); 183 `.chiplet` across the umbrella tree with build trees included (no lone CR, none of the other three). All three are committed machine-written output, which is the population most likely to be clean. A consumer that keeps reading them is incomplete, never wrong. |
| Rule 4 stops refusing an unrecognised `interfaces[].type`, in both readers | this slice (SPEC-32) | The accepted set strictly GROWS, which is the direction this table is usually asked about in reverse. No document legal today becomes illegal, nothing on disk changes, and a consumer that keeps its own refusal stays correct, so it is not even a format MINOR: it is a READER minor and is recorded as one above. It is also the correction of a rule that contradicted its own text here: rule 4 sat under "enforced by the reference VALIDATOR" while the C++ reference enforced it in the PARSER, where the validate flag did not reach it. The one thing this does NOT do is settle `solder_bump`: adding an enum member is still a MINOR by the rule above, the format 1.1 stamp is still owed, and a producer still may not emit it in a 1.0 document. |
| A `die_array`'s anchor defined: it applies per element, and `array.start_position` places the first element's anchor point | this slice (SPEC-30) | A rule the format never had, so nothing that had a meaning changed meaning; a consumer that had picked the other reading was guessing at a gap, not following the format. It is a MINOR and not a MAJOR only because the population is TWO and both are in reach: this repository's `v1_0_all_blocks.chiplet` declared `gds_origin` on its array and moves to `bbox_center` in the same commit, and the studio's `with_components.chiplet` declares no anchor and does not move. Had the fixture stayed, the ruling would have restated an existing document's meaning after the fact, which is the one thing the sentence above forbids. The reading itself is the only one under which `anchor` keeps ONE definition: `start_position` is a `$ref` to the same `position3d` as `position`, so the rule attaches to that definition, and the alternative would make `anchor` the single field whose effect depends on the component's type. |
