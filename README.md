...
| `docs/VERSION_POLICY.md` | The one version rule for the `.chiplet` `format_version` and every governed sidecar: what a consumer accepts, what bumps MAJOR/MINOR/PATCH, and how a key is deprecated. |
| `schemas/chiplet.schema.json` | Machine-readable schema of the `.chiplet` root document, normative for structure (the reference reader stays normative for semantics). |
| `schemas/*.schema.json` | Machine-readable JSON Schemas for the sidecars (boundary manifest, interconnect methods, io_pads, pins, black-box padmap, layers, rule params, interconnect rules). |
| `schemas/chiplet_pads.json` | Pad-layer vocabulary for black-box chiplets (GDS 205/0, 205/25, 206/0). A byte-identical mirror of `IHP-Open-ADK/config/chiplet_pads.json`, which owns it. Its `version` tracks the ADK release, not the format version, so bump it there and re-copy; do not edit this file on its own. |
